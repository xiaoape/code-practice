...
// 实现原理：使用数组模拟队列，首先将根结点归入队列。当队列不为空时，执行循环：取出队列的一个结点，如果该节点有左子树，则将该节点的左子树存入队列；
...
var levelOrder = function (root, res = [], level = 0) {
  // 不能返回level，因为level是一个基本类型，所以不会改变，但是res是一个引用类型
  if (root === null) {
    return [];
  }
...
// 练习地址：https://leetcode.cn/problems/binary-tree-level-order-traversal/description/