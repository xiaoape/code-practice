  ...
  function resolve(value) {
    setTimeout(function () {
      if (self.state === PENDING) {
        ...
        self.value = value;
        // promise是一次性的 onFulfilledFn是存储了所有可被执行的fulfilled函数
        // 这里并不是这一个最初的promise是fulfilled后，就将所有then函数的第一个回调函数执行，而是then函数每次都基于上一个then函数返回的promise的状态来决定
  ...
  function reject(reason) {
    setTimeout(function () {
      if (self.state === PENDING) {
        self.state = REJECTED;
        self.reason = reason;
        // promise是一次性的 onRejectedFn是存储了所有可被执行的rejected函数
